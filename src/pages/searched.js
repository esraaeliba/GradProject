...
const LaptopPageNew = () => {
    ...
    //     try {
    //         const response = await fetch(`http://localhost:3001/product/search/${keyword}`); // Pass keyword as a query parameter
    ...
    const getLaptopsFromApi = (keyword) => {
        fetch(`http://localhost:3001/product/search/${keyword}`)
            .then((res) => res.json())
            .then((json) => {
        ...
        <div className={styles.laptopPageNew}>
            ...
            <form className={styles.searchBar}>
                <div className={styles.bar}>
                    <button className={styles.barItem} onChange={(e) => handleSearch(e.target.value)} type="submit" />
                    <input
                        type="text"
                        name="search"
                        placeholder="Search"
